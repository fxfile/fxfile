...
void AddressBar::doBaseItems(void)
{
    ...
    sCbItemData->mFullPidl    = fxb::ConcatPidls(sFullPidl, sFullPidl);
...
void AddressBar::doBaseItemsInDesktop(void)
{
    ...
    {
        if (mShcnDesktopId == 0)
            mShcnDesktopId = fxb::ShellChangeNotify::instance().registerWatch(this, WM_SHCN_DESKTOP, sFullPidl, sEventMask, XPR_FALSE);
...
void AddressBar::doBaseItemsInComputer(void)
{
    ...
    if (SUCCEEDED(sHResult) && XPR_IS_NOT_NULL(sFullPidl))
    {
        ...
        if (mShcnComputerId == 0)
            mShcnComputerId = fxb::ShellChangeNotify::instance().registerWatch(this, WM_SHCN_COMPUTER, sFullPidl, sEventMask, XPR_FALSE);
                                ...
                                xpr_uint_t     aType)
{
    ...
    while (S_OK == sEnumIdList->Next(1, &sPidl, &sFetched))
    {
        ...
        {
            fxb::GetName(aShellFolder, sPidl, SHGDN_INFOLDER, sName);
        ...
        sCbItemData->mFullPidl        = fxb::ConcatPidls(aFullPidl, sPidl);
...
void AddressBar::insertItems(CbItemDataDeque &aCbItemDataDeque, xpr_uint_t aInsert, xpr_bool_t aForParsing)
{
    ...
    for (; sIterator != aCbItemDataDeque.end(); ++sIterator)
    {
        ...
        if (XPR_IS_TRUE(aForParsing) && (sIterator+1) == aCbItemDataDeque.end())
            fxb::GetName(sCbItemData->mShellFolder, sCbItemData->mPidl, SHGDN_FORPARSING, sName);

        if (sName[0] == XPR_STRING_LITERAL('\0'))
            fxb::GetName(sCbItemData->mShellFolder, sCbItemData->mPidl, SHGDN_INFOLDER, sName);
        ...
        sComboBoxExItem.iImage         = fxb::GetItemIconIndex(sCbItemData->mFullPidl, XPR_FALSE);
        sComboBoxExItem.iSelectedImage = fxb::GetItemIconIndex(sCbItemData->mFullPidl, XPR_TRUE);
...
void AddressBar::OnSelEndOK(void)
{
    ...
    LPITEMIDLIST sFullPidl = fxb::CopyItemIDList(sCbItemData->mFullPidl);
...
xpr_bool_t AddressBar::explore(LPITEMIDLIST aFullPidl, xpr_bool_t aNotifyBuddy)
{
    ...
    if (XPR_IS_FALSE(sResult))
    {
        if (XPR_IS_NOT_NULL(mOldSelFullPidl))
        {
            ...
            if (XPR_IS_TRUE(sResult))
                aFullPidl = fxb::CopyItemIDList(mOldSelFullPidl);
        }
    }
    ...
    {
        COM_FREE(mOldSelFullPidl);
        mOldSelFullPidl = fxb::CopyItemIDList(aFullPidl);
    }
...
xpr_bool_t AddressBar::exploreItem(LPITEMIDLIST aFullPidl, xpr_bool_t aNotifyBuddy)
{
    ...
    if (sIndex < 0)
    {
        ...
        sCount = fxb::GetItemIDCount(aFullPidl);
        for (i = 0; i < sCount; ++i)
        {
        ...
        aFullPidl = fxb::CopyItemIDList(aFullPidl);
        sCount = fxb::GetItemIDCount(aFullPidl);
        ...
        for (i = sCount - 1; i >= sItemIDList; --i)
        {
            ...
            sCbItemData = new ABITEMDATA;
            sCbItemData->mShellFolder     = sShellFolder;
            sCbItemData->mPidl            = fxb::CopyItemIDList(sPidl);
            sCbItemData->mFullPidl        = fxb::CopyItemIDList(aFullPidl);
            sCbItemData->mShellAttributes = sShellAttributes;
...
xpr_bool_t AddressBar::explore(const xpr_tchar_t *aFullPath, xpr_bool_t aNotifyBuddy)
{
    ...
    if (fxb::GetDispFullPidl(aFullPath, &aFullPidl) == XPR_FALSE)
        return XPR_FALSE;
...
xpr_bool_t AddressBar::execute(const xpr_tchar_t *aPath)
{
    ...
    if (sPath[0] == '%')
    {
        sResult = fxb::GetEnvRealPidl(sPath, &sFullPidl);
    }

    // 3. file system path (with URL path)
    if (fxb::IsExistFile(sPath) == XPR_TRUE)
    {
        if (_tcslen(sPath) == 2)
            _tcscat(sPath, XPR_STRING_LITERAL("\\"));

        sResult = SUCCEEDED(fxb::SHGetPidlFromPath(sPath, &sFullPidl));
    }
    ...
    if (XPR_IS_FALSE(sResult))
    {
        ...
        if (sIndex >= 0)
        {
            ...
            if (XPR_IS_NOT_NULL(sCbItemData))
            {
                sFullPidl = fxb::CopyItemIDList(sCbItemData->mFullPidl);
    ...
    // 6. full path
    if (XPR_IS_FALSE(sResult))
        sResult = fxb::GetDispFullPidl(sPath, &sFullPidl);
    ...
    if (XPR_IS_TRUE(sResult))
    {
        ...
        {
            sResult = sExecFile = fxb::ExecFile(sPath, XPR_NULL);
        }
        else
        {
...
xpr_sint_t AddressBar::searchFullPath(const xpr_tchar_t *aFullPath)
{
    ...
    for (i = 0; i < sCount; ++i)
    {
        ...
        fxb::GetDispFullPath(sCbItemData->mFullPidl, sFullPath);
...
void AddressBar::OnShcnRenameItem(xpr_uint_t aType, fxb::Shcn *aShcn)
{
    xpr_tchar_t sFullPath[XPR_MAX_PATH * 2 + 1] = {0};
    ...
    sHResult = fxb::SH_BindToParent(aShcn->mPidl2, IID_IShellFolder, (LPVOID *)&sShellFolder, (LPCITEMIDLIST *)&sPidl);
    if (SUCCEEDED(sHResult) && XPR_IS_NOT_NULL(sShellFolder) && XPR_IS_NOT_NULL(sPidl))
    {
        ...
        sCbItemData->mShellFolder     = sShellFolder;
        sCbItemData->mPidl            = fxb::CopyItemIDList(sPidl);
        sCbItemData->mFullPidl        = fxb::CopyItemIDList(aShcn->mPidl2);
        sCbItemData->mShellAttributes = sShellAttributes;
...
void AddressBar::OnShcnDeleteItem(xpr_uint_t aType, LPITEMIDLIST aFullPidl)
{
    ...
    fxb::GetName(aFullPidl, SHGDN_FORPARSING, szPath1);
    ...
    for (i = sCount - 1; i >= 0; --i)
    {
        ...
        fxb::GetName(sCbItemData->mShellFolder, sCbItemData->mPidl, SHGDN_FORPARSING, szPath2);
...
void AddressBar::OnShcnUpdateDir(xpr_uint_t aType, LPITEMIDLIST aFullPidl)
{
    ...
    if (sIndex >= 0)
    {
        LPABITEMDATA sCbItemData = (LPABITEMDATA)GetItemData(sIndex);
        if (XPR_IS_NOT_NULL(sCbItemData))
            sCurSelFullPidl = fxb::CopyItemIDList(sCbItemData->mFullPidl);
    }
