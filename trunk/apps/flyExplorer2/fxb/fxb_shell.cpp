...
#include "fxb_vwin32.h"    // 9x I/O Control - Drive Type
...
HRESULT SHGetPidlFromPath(const xpr_tchar_t *aPath, LPITEMIDLIST *aFullPidl)
{
    if (XPR_IS_NULL(aFullPidl))
        return E_FAIL;

    *aFullPidl = IL_CreateFromPath(aPath);

    return XPR_IS_NOT_NULL(*aFullPidl) ? S_OK : E_FAIL;
}


HRESULT SHGetSimplePidlFromPath(const xpr_tchar_t *aPath, LPITEMIDLIST *aPidl)
{
    if (XPR_IS_NULL(aPath) || XPR_IS_NULL(aPidl))
        return E_FAIL;
...
LPITEMIDLIST Path2Pidl(const xpr_tchar_t *aPath)
{
    ...
    {
        return SHGetPidlFromPath(aPath);
    }
...
xpr_bool_t Pidl2Path(LPITEMIDLIST aFullPidl, xpr_tchar_t *aPath, xpr_bool_t aOnlyFileSystemPath)
{
    if (XPR_IS_NULL(aPath))
        return XPR_FALSE;
...
xpr_bool_t Pidl2Path(LPITEMIDLIST aFullPidl, std::tstring &aPath, xpr_bool_t aOnlyFileSystemPath)
{
...
xpr_bool_t GetFolderPath(LPCITEMIDLIST aFullPidl, xpr_tchar_t *aPath, xpr_ulong_t aShellAttributes)
{
    ...
    HRESULT sHResult = SH_BindToParent(aFullPidl, IID_IShellFolder, (LPVOID *)&sShellFolder, (LPCITEMIDLIST *)&sPidl);
...
xpr_bool_t GetFolderPath(LPCITEMIDLIST aFullPidl, std::tstring &aPath, xpr_ulong_t aShellAttributes)
{
    aPath.clear();
...
xpr_bool_t GetFolderPath(LPSHELLFOLDER aShellFolder, LPITEMIDLIST aPidl, xpr_tchar_t *aPath, xpr_ulong_t aShellAttributes)
{
...
xpr_bool_t GetFolderPath(LPSHELLFOLDER aShellFolder, LPITEMIDLIST aPidl, std::tstring &aPath, xpr_ulong_t aShellAttributes)
{
    aPath.clear();
...
void GetFullPath(LPITEMIDLIST aFullPidl, std::tstring &aFullPath)
{
    ...
    LPITEMIDLIST sFullPidl2 = CopyItemIDList(aFullPidl);
...
void GetDispFullPath(LPITEMIDLIST aFullPidl, std::tstring &aPath)
{
    ...
    std::tstring sName;
    LPITEMIDLIST sFullPidl2 = IL_Clone(aFullPidl);
...
xpr_bool_t GetDispFullPidl(const xpr_tchar_t *aFullPath, LPITEMIDLIST *aFullPidl)
{
    ...
    for (i = 0; i < sCount; ++i) // preventing for infinite loop
    {
        ...
        else
        {
            ...
            {
                LPITEMIDLIST sFullPidl2 = ConcatPidls(sFullPidl, sPidl);
                COM_FREE(sFullPidl);
                COM_FREE(sPidl);
...
//
// sShellFolder + sPidl <-> sFullPidl
//
LPITEMIDLIST SH_GetRealIDL(LPSHELLFOLDER sShellFolder, LPITEMIDLIST aPidl)
{
    HRESULT sHResult;
    LPITEMIDLIST sFullPidl = XPR_NULL;

    sHResult = SH_GetRealIDL(sShellFolder, aPidl, &sFullPidl);
    if (FAILED(sHResult))
        return XPR_NULL;
...
HRESULT SH_GetRealIDL(LPSHELLFOLDER aShellFolder, LPCITEMIDLIST aSimplePidl, LPITEMIDLIST *aFullPidl)
{
    if (XPR_IS_NULL(aShellFolder) || XPR_IS_NULL(aSimplePidl) || XPR_IS_NULL(aFullPidl))
    {
    ...
    if (SUCCEEDED(sHResult))
    {
        ...
        if (SUCCEEDED(sHResult))
        {
            ...
            *aFullPidl = ILCombine(sFolderPidl, sChildPidl);

            if (*aFullPidl == XPR_NULL)
                sHResult = E_OUTOFMEMORY;
...
HRESULT SH_BindToParent(LPCITEMIDLIST aPidl, REFIID riid, LPVOID *ppv, LPCITEMIDLIST *aLastPidl)
{
    ...
    LPITEMIDLIST sChildPidl, sParentPidl;
    HRESULT sHResult = E_FAIL;

    *ppv = XPR_NULL;
    if (XPR_IS_NOT_NULL(aLastPidl))
        *aLastPidl = XPR_NULL;

    if (IsPidlSimple(aPidl) == XPR_TRUE)
    {
        // we are on desktop level
        if (XPR_IS_NOT_NULL(aLastPidl))
            *aLastPidl = IL_FindLastID(aPidl);

        sHResult = SHGetDesktopFolder((IShellFolder**)ppv);
    }
    else
    {
        sChildPidl = IL_FindLastID(aPidl);
        sParentPidl = IL_Clone(aPidl);
        IL_RemoveLastID(sParentPidl);

        sHResult = SHGetDesktopFolder(&sShellFolder);
        if (SUCCEEDED(sHResult))
            sHResult = sShellFolder->BindToObject(sParentPidl, XPR_NULL, riid, ppv);

        if (SUCCEEDED(sHResult) && XPR_IS_NOT_NULL(aLastPidl))
            *aLastPidl = sChildPidl;
...
HICON GetItemIcon(LPITEMIDLIST aFullPidl, xpr_bool_t aOpen, xpr_bool_t aLarge)
{
    SHFILEINFO sShFileInfo;
    xpr_uint_t sFlags;
    sShFileInfo.hIcon = XPR_NULL;
...
HICON GetItemIcon(LPSHELLFOLDER aShellFolder, LPITEMIDLIST aPidl, xpr_bool_t aOpen, xpr_bool_t aLarge)
{
    if (XPR_IS_NULL(aShellFolder) || XPR_IS_NULL(aPidl))
        return XPR_NULL;

    LPITEMIDLIST sFullPidl = XPR_NULL;
    HRESULT sHResult = SH_GetRealIDL(aShellFolder, aPidl, &sFullPidl);
    if (FAILED(sHResult))
        return XPR_NULL;
...
xpr_sint_t GetItemIconIndex(LPITEMIDLIST aFullPidl, xpr_bool_t aOpen)
{
    SHFILEINFO sShFileInfo;
...
xpr_sint_t GetItemIconIndex(LPSHELLFOLDER aShellFolder, LPITEMIDLIST aPidl, xpr_bool_t aOpenIcon)
{
    ...
    {
        LPITEMIDLIST sFullPidl = XPR_NULL;
        sHResult = SH_GetRealIDL(aShellFolder, aPidl, &sFullPidl);
...
xpr_sint_t GetItemIconOverlayIndex(const xpr_tchar_t *aPath, xpr_bool_t aIconOverlay)
{
    ...
    LPITEMIDLIST sFullPidl = SHGetPidlFromPath(aPath);
    if (XPR_IS_NULL(sFullPidl))
        return -1;
...
xpr_sint_t GetItemIconOverlayIndex(LPITEMIDLIST aFullPidl, xpr_bool_t aIconOverlay)
{
    if (XPR_IS_NULL(aFullPidl))
        return -1;
    ...
    HRESULT sHResult;
    sHResult = SH_BindToParent(aFullPidl, IID_IShellFolder, (LPVOID *)&sShellFolder, (LPCITEMIDLIST *)&sPidl);
    if (SUCCEEDED(sHResult))
...
xpr_sint_t GetItemIconOverlayIndex(LPSHELLFOLDER aShellFolder, LPITEMIDLIST aPidl, xpr_bool_t aIconOverlay)
{
    if (XPR_IS_NULL(aShellFolder) || XPR_IS_NULL(aPidl))
        return -1;
...
xpr_bool_t GetItemAttributes(LPITEMIDLIST aFullPidl, xpr_ulong_t &aShellAttributes)
{
    ...
    xpr_bool_t sResult = GetItemAttributes(sShellFolder, sPidl, aShellAttributes);
    if (XPR_IS_TRUE(sResult))
    {
...
xpr_bool_t IsFileSystem(LPITEMIDLIST aFullPidl)
{
    ...
    xpr_bool_t sResult = IsFileSystem(sShellFolder, sPidl);
    COM_RELEASE(sShellFolder);
    return sResult;
}
...
xpr_bool_t IsFileSystemFolder(LPITEMIDLIST aFullPidl)
{
    ...
    xpr_bool_t sResult = IsFileSystemFolder(sShellFolder, sPidl);
    COM_RELEASE(sShellFolder);
    return sResult;
}
...
xpr_bool_t IsVirtualItem(LPITEMIDLIST aFullPidl)
{
    ...
    LPITEMIDLIST sPidl = XPR_NULL;
    HRESULT sHResult = SH_BindToParent(aFullPidl, IID_IShellFolder, (LPVOID *)&sShellFolder, (LPCITEMIDLIST *)&sPidl);
    if (FAILED(sHResult) || XPR_IS_NULL(sShellFolder) || XPR_IS_NULL(sPidl))
        return XPR_FALSE;

    xpr_bool_t sResult = IsVirtualItem(sShellFolder, sPidl);
    COM_RELEASE(sShellFolder);
    return sResult;
}
...
const xpr_tchar_t *GetPathToFileName(const xpr_tchar_t *aPath, xpr_sint_t *aFileNameLen)
{
    if (XPR_IS_NULL(aPath))
        return XPR_NULL;
    ...
    if (XPR_IS_NULL(sExt))
    {
        if (XPR_IS_NOT_NULL(aFileNameLen))
            *aFileNameLen = sLen;
    ...
    if (XPR_IS_NOT_NULL(aFileNameLen))
        *aFileNameLen = sLen - (xpr_sint_t)_tcslen(sExt);
...
xpr_bool_t GetPathFileNameExt(const xpr_tchar_t *aPath, xpr_tchar_t *aFileName, xpr_tchar_t *aExt)
{
    ...
    sLen -= (xpr_sint_t)_tcslen(sExt);

    if (XPR_IS_NOT_NULL(aFileName))
    {
        _tcsncpy(aFileName, aPath, sLen);
        aFileName[sLen] = XPR_STRING_LITERAL('\0');
    }
                    ...
                    xpr_sint_t         aShowCmd)
{
    ...
    sShellExecuteInfo.lpIDList     = aFullPidl;
