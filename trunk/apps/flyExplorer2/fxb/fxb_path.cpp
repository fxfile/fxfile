...
namespace fxb
{
LPITEMIDLIST GetEnvPidl(const std::tstring &aEnv, xpr_uint_t *aCSIDL)
{
...
xpr_bool_t GetEnvPidl(const std::tstring &aEnv, LPITEMIDLIST *aPidl, xpr_uint_t *aCSIDL)
{
    return EnvPath::instance().getPidl(aEnv, aPidl, aCSIDL);
}

xpr_bool_t GetEnvPath(const std::tstring &aEnv, std::tstring &aPath, xpr_uint_t *aCSIDL)
{
...
static inline xpr_bool_t GetEnv(const std::tstring &aEnvPath, std::tstring &aEnv)
{
    aEnv.clear();
...
xpr_bool_t GetEnvRealPidl(const std::tstring &aEnvPath, LPITEMIDLIST *aPidl)
{
...
LPITEMIDLIST GetEnvRealPidl(const std::tstring &aEnvPath)
{
    ...
    if (sSubPath.empty() == true)
    {
        ...
        return SHGetPidlFromPath(aEnvPath.c_str());
    }
    ...
    while (*sSplit != 0)
    {
        ...
        sFullPidl2 = ConcatPidls(sFullPidl, sPidl);
        COM_FREE(sFullPidl);
        COM_FREE(sPidl);
...
void CombinePath(std::tstring &aPath, const std::tstring &aDir, const std::tstring &aFileName, const xpr_tchar_t *aExt)
{
    aPath.clear();
...
{
    xpr_size_t sFind = 0;
    while ((sFind = aPath.find(XPR_STRING_LITERAL('\\'), sFind)) != std::tstring::npos)
    {
...
static xpr_bool_t _fxIsEqualPath(std::tstring aPath1, std::tstring aPath2, xpr_bool_t aCase)
{
...
xpr_bool_t IsEqualPath(const xpr_tchar_t *aPath1, const xpr_tchar_t *aPath2, xpr_bool_t aCase)
{
    ...
    std::tstring sPath1(aPath1);
    std::tstring sPath2(aPath2);

    return _fxIsEqualPath(sPath1, sPath2, aCase);
}

xpr_bool_t IsEqualPath(const std::tstring &aPath1, const std::tstring &aPath2, xpr_bool_t aCase)
{
    std::tstring sPath1(aPath1);
    std::tstring sPath2(aPath1);
...
xpr_bool_t VerifyFileName(const xpr_tchar_t *aFileName)
{
    ...
    std::tstring sFileName(aFileName);
