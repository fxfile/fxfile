...
unsigned CrcCreate::OnEntryProc(void)
{
    ...
    if (XPR_IS_FALSE(mEach))
    {
        ...
        if (XPR_RCODE_IS_SUCCESS(sRcode))
        {
            ...
            if (mMethod == 0)
            {
                sTextFileWriter->writeLine(XPR_STRING_LITERAL("; Generated by WIN-SFV32 v1.0 (Compatible: flyExplorer)"));
            }
        }
    }
    ...
    if (XPR_IS_TRUE(mEach) || (XPR_IS_FALSE(mEach) && sFileIo.isOpened() == XPR_TRUE))
    {
        ...
        xpr_char_t sCrcMd5A[0xff]; // md5
        xpr_tchar_t sCrcMd5[0xff]; // md5
        ...
        for (i = 0; sIterator != mPathDeque.end(); ++sIterator, ++i)
        {
            ...
            if (mMethod == 0)
            {
                getFileCrcSfv(sPath.c_str(), &sCrcSfv);
            }
            else
            {
                getFileCrcMd5(sPath.c_str(), sCrcMd5A);

                sInputBytes = strlen(sCrcMd5A) * sizeof(xpr_char_t);
                sOutputBytes = 0xfe * sizeof(xpr_tchar_t);
                XPR_MBS_TO_TCS(sCrcMd5A, &sInputBytes, sCrcMd5, &sOutputBytes);
                sCrcMd5[sOutputBytes / sizeof(xpr_tchar_t)] = 0;
            }
            ...
            if (mMethod == 0)
            {
                sTextFileWriter->writeFormatLine(XPR_STRING_LITERAL("%s %08X"), sFileName.c_str(), sCrcSfv);
            }
            else
            {
                sTextFileWriter->writeFormatLine(XPR_STRING_LITERAL("%s %s"), _tcsupr(sCrcMd5), sFileName.c_str());
            }
