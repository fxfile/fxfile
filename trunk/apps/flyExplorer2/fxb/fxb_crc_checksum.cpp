...
//----------------------------------------------------------------------
static const xpr_ulong_t kCrcTable[256] = {
...
static xpr_ulong_t getCrc32(xpr_ulong_t aCrc, xpr_uchar_t *aBuf, xpr_sint_t aLen)
{
    if ((aLen > 0) && (aBuf != XPR_NULL))
    {
        do {
            aCrc = ((aCrc >> 8) & 0xFFFFFF) ^ kCrcTable[(xpr_uchar_t)( (aCrc & 0xff) ^ *(aBuf++))];
        } while (--aLen);
    }
...
xpr_sint_t getFileCrcSfv(const xpr_tchar_t *aPath, xpr_char_t *aCrcVal)
{
    ...
    xpr_ulong_t sCrcVal = 0;
    xpr_sint_t sResult = getFileCrcSfv(aPath, &sCrcVal);
    if (sResult == CRC_ERR_NO)
        sprintf(aCrcVal, "%08x", sCrcVal);

    return sResult;
}

//----------------------------------------------------------------------
// Digest Message 5 (MD5)
//----------------------------------------------------------------------
xpr_sint_t getFileCrcMd5(const xpr_tchar_t *aPath, xpr_char_t *aCrcVal)
{
    ...
    if (XPR_IS_NOT_NULL(sCrcVal))
    {
        strcpy(aCrcVal, sCrcVal);
