...
void SyncDlg::OnPath1Browse(void)
{
    ...
    sOldFullPidl = fxb::SHGetPidlFromPath(sPath);
...
void SyncDlg::OnNMCustomdrawList(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    *pResult = 0;
    ...
    fxb::SyncItem *sSyncItem = (fxb::SyncItem *)pLVCD->nmcd.lItemlParam;

    xpr_uint_t sSync = fxb::SyncNone;
    ...
    else if (pLVCD->nmcd.dwDrawStage == CDDS_ITEMPREPAINT)
    {
        switch (sSync)
        {
        case fxb::SyncToRight:    pLVCD->clrText = RGB(0,128,0); break;
        case fxb::SyncToLeft:     pLVCD->clrText = RGB(0,0,255); break;
        case fxb::SyncNotEqualed: pLVCD->clrText = RGB(255,0,0); break;
        }
    ...
    else if (pLVCD->nmcd.dwDrawStage == CDDS_ITEMPOSTPAINT)
    {
        ...
        if (sDc != XPR_NULL)
        {
            ...
            if (sHeaderCtrl != XPR_NULL)
            {
                ...
                switch (sSync)
                {
                case fxb::SyncFailed:     sImage = 0; break;
                case fxb::SyncEqualed:    sImage = 1; break;
                case fxb::SyncNotEqualed: sImage = 2; break;
                case fxb::SyncToRight:    sImage = 3; break;
                case fxb::SyncToLeft:     sImage = 4; break;
                }
...
void SyncDlg::OnLvnGetdispinfoList(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    *pResult = 0;
    ...
    fxb::SyncItem *sSyncItem = (fxb::SyncItem *)pDispInfo->item.lParam;
    if (sSyncItem == XPR_NULL)
        return;
    ...
    if (XPR_TEST_BITS(sLvItem.mask, LVIF_TEXT))
    {
            ...
            (sLvItem.iSubItem == 5 && sSyncItem->isExistRight() == XPR_TRUE))
        {
            ...
            {
                fxb::GetFormatedNumber(sSyncItem->mFileSize[sIndex], sLvItem.pszText, 200);
            }
        }
...
void SyncDlg::view(void)
{
    ...
    for (i = 0; i < sCount; ++i)
    {
        ...
        if (sSyncItem->mExist == fxb::CompareExistNone)
            continue;
...
void SyncDlg::scan(void)
{
    ...
    sIncorrectedPath[0] = sLen[0] <= 0 || !fxb::IsExistFile(sPath[0]);
    sIncorrectedPath[1] = sLen[1] <= 0 || !fxb::IsExistFile(sPath[1]);
    ...
    std::tstring sIncFilter = sTempIncFilter;
    std::tstring sExcFilter = sTempExcFilter;
    fxb::trim(sIncFilter);
    fxb::trim(sExcFilter);
    ...
    xpr_uint_t sExcExist = fxb::CompareExistNone;
    if (sExcludeEqual == XPR_TRUE) sExcExist = fxb::CompareExistEqual;
    if (sExcludeOther == XPR_TRUE) sExcExist = fxb::CompareExistOther;
...
void SyncDlg::compare(void)
{
    ...
    mSyncDirs->setContents((sContents == XPR_TRUE) ? fxb::CompareContentsBytes : fxb::CompareContentsNone);
...
void SyncDlg::OnCompare(void)
{
    ...
    if (mSyncDirs != XPR_NULL)
    {
        fxb::SyncDirs::Status sStatus = fxb::SyncDirs::StatusNone;
        mSyncDirs->getStatus(sStatus);

        if (sStatus == fxb::SyncDirs::StatusScaning || sStatus == fxb::SyncDirs::StatusComparing)
        {
            mSyncDirs->stop();
            mStop = XPR_TRUE;
            return;
...
void SyncDlg::OnSynchronize(void)
{
    ...
    if (mSyncDirs != XPR_NULL)
    {
        ...
        if (sStatus == fxb::SyncDirs::StatusSynchronizing)
        {
            mSyncDirs->stop();
            mStop = XPR_TRUE;
            return;
...
LRESULT SyncDlg::OnSyncStatus(WPARAM wParam, LPARAM lParam)
{
    ...
    switch (sStatus)
    {
    case fxb::SyncDirs::StatusScanCompleted:
        {
            xpr_size_t sFolderCount = (xpr_size_t)lParam;

            xpr_tchar_t sFolderCountText[0xff] = {0};
            fxb::GetFormatedNumber(sFolderCount, sFolderCountText, 0xfe);
    ...
    case fxb::SyncDirs::StatusCompareCompleted:
        {
            ...
            fxb::GetFormatedNumber(sTotalCount,      sTotalCountText,      0xfe);
            fxb::GetFormatedNumber(sDirCount,        sDirCountText,        0xfe);
            fxb::GetFormatedNumber(sFileCount,       sFileCountText,       0xfe);
            fxb::GetFormatedNumber(sEqualCount,      sEqualCountText,      0xfe);
            fxb::GetFormatedNumber(sNotEqualCount,   sNotEqualCountText,   0xfe);
            fxb::GetFormatedNumber(sLeftExistCount,  sLeftExistCountText,  0xfe);
            fxb::GetFormatedNumber(sRightExistCount, sRightExistCountText, 0xfe);
            fxb::GetFormatedNumber(sFailCount,       sFailCountText,       0xfe);
    ...
    case fxb::SyncDirs::StatusSynchronizeCompleted:
        {
            KillTimer(TM_SYNC_STATUS);
            ...
            fxb::GetFormatedNumber(sSyncCount, sSyncCountText, 0xfe);
...
void SyncDlg::OnTimer(UINT_PTR nIDEvent)
{
    if (nIDEvent == TM_SYNC_STATUS)
    {
        ...
        switch (sStatus)
        {
        case fxb::SyncDirs::StatusScaning:
            {
        ...
        case fxb::SyncDirs::StatusComparing:
            {
                fxb::GetFormatedNumber(sCount, sCountText, 0xfe);
        ...
        case fxb::SyncDirs::StatusSynchronizing:
            {
                fxb::GetFormatedNumber(sCount, sCountText, 0xfe);
...
void SyncDlg::OnContextMenu(CWnd *pWnd, CPoint pt)
{
    ...
    sSyncItem = (fxb::SyncItem *)mListCtrl.GetItemData(sIndex);
    if (sSyncItem == XPR_NULL)
        return;
...
void SyncDlg::OnPrevDiff(void)
{
    ...
    for (; sIndex >= 0; --sIndex)
    {
        ...
        sSync = sSyncItem->getSync();
        if (sSync != fxb::SyncNone && sSync != fxb::SyncEqualed)
            break;
    }
