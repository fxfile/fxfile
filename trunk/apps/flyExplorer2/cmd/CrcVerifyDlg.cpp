...
void CrcVerifyDlg::OnOK(void) 
{
    ...
    {
        if (mCrcVerify->getStatus() == fxb::CrcVerify::StatusVerifying)
        {
            mCrcVerify->Stop();
            return;
        }
...
LRESULT CrcVerifyDlg::OnFinalize(WPARAM wParam, LPARAM lParam)
{
...

    {
        ...
        for (i = 0; i < sCount; ++i)
        {
            sVerifyFile = (fxb::CrcVerify::VerifyFile *)mListCtrl.GetItemData(i);
            sText = XPR_NULL;
    ...
    switch (sStatus)
    {
    case fxb::CrcVerify::StatusVerifyCompleted:
        {
            xpr_tchar_t sStatusText[0xff] = {0};
    ...
    case fxb::CrcVerify::StatusStopped:
        {
            xpr_tchar_t sStatusText[0xff] = {0};
