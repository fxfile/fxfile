...
xpr_bool_t DriveDlg::OnInitDialog(void) 
{
    ...
    mTreeCtrl.SetImageList(&fxb::SysImgListMgr::instance().mSysImgList16, TVSIL_NORMAL);
...
void DriveDlg::OnRefresh(void)
{
    ...
    fxb::GetDriveStrings(sDrivePath);
...
void DriveDlg::addDrive(xpr_tchar_t *aDrivePath)
{
    ...
    HRESULT sResult = fxb::SHGetPidlFromPath(aDrivePath, &sFullPidl);
    if (FAILED(sResult))
        return;

    xpr_sint_t sImage = fxb::GetItemIconIndex(sFullPidl, XPR_FALSE);
...
void DriveDlg::OnRclickDrvTree(NMHDR *pNMHDR, LRESULT *pResult)
{
    *pResult = 0;
    ...
    LPITEMIDLIST sFullPidl = fxb::SHGetPidlFromPath(sDrivePath);
