...
#include "stdafx.h"
#include "FileCombineDlg.h"

#include "fxb/fxb_file_combine.h"
...
void FileCombineDlg::OnOK(void) 
{
    if (mFileCombine != XPR_NULL)
    {
        if (mFileCombine->GetStatus() == fxb::FileCombine::StatusCombining)
        {
            mFileCombine->Stop();
            return;
        }
    ...
    xpr_tchar_t *sExt = (xpr_tchar_t *)fxb::GetFileExt(sDestPath);
    if (sExt != XPR_NULL)
    {
        _stscanf(sExt+1, XPR_STRING_LITERAL("%d"), &sIndex);
        if (sIndex != -1)
    ...
    mFileCombine = new fxb::FileCombine;
            ...
            *sExt = XPR_STRING_LITERAL('\0');
            ...
            if (i >= 1)
            {
                ...
                for (; ; ++i)
                {
                    ...
                    if (fxb::IsExistFile(sCombinePath) == XPR_FALSE)
                        break;
    ...
    else
    {
        ...
        {
        case fxb::FileCombine::StatusFileNotExist:
            {
        ...
        case fxb::FileCombine::StatusDestDirNotExist:
            {
...
LRESULT FileCombineDlg::OnFinalize(WPARAM wParam, LPARAM lParam)
{
    ...
    xpr_size_t sCombined = 0;
    fxb::FileCombine::Status sStatus;
    ...
    switch (sStatus)
    {
    case fxb::FileCombine::StatusNotCombine:
        {
            const xpr_tchar_t *sMsg = theApp.loadString(XPR_STRING_LITERAL("popup.file_combine.msg.no_combine"));
            MessageBox(sMsg, XPR_NULL, MB_OK | MB_ICONSTOP);
            break;
    ...
    case fxb::FileCombine::StatusNotWritable:
        {
    ...
    case fxb::FileCombine::StatusCombineCompleted:
        {
            xpr_tchar_t sStatusText[0xff] = {0};
    ...
    case fxb::FileCombine::StatusStopped:
        {
            xpr_tchar_t sStatusText[0xff] = {0};
    ...
    case fxb::FileCombine::StatusInvalidCrcCode:
        {
            xpr_tchar_t sStatusText[0xff] = {0};
