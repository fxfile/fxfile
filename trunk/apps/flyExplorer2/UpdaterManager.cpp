...
void UpdaterManager::writeUpdaterConf(Option::Config &aConfig)
{
    ...
    sIniFile.setComment(XPR_STRING_LITERAL("fxfile updater configuration file"));

    fxb::IniFile::Section *sSection;

    sSection = sIniFile.addSection(kUpdaterSection);
    XPR_ASSERT(sSection != XPR_NULL);

    xpr_tchar_t sAppVer[0xff] = {0};
    getAppVer(sAppVer);

    sIniFile.setValueB(sSection, kUpdaterConfKeyEnable,        aConfig.mUpdateCheckEnable);
    sIniFile.setValueS(sSection, kUpdaterConfKeyCurVer,        sAppVer);
    sIniFile.setValueB(sSection, kUpdaterConfKeyCheckMinorVer, aConfig.mUpdateCheckMinorVer);

    sIniFile.writeFile(xpr::CharSetUtf8);
}
