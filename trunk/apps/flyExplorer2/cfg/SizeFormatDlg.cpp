...
void SizeFormatDlg::OnOK(void) 
{
    ...
    for (i = 0; i < sCount; ++i)
    {
        sItem = (fxb::SizeFormat::Item *)mListCtrl.GetItemData(i);
        if (sItem == XPR_NULL)
            continue;
...
void SizeFormatDlg::OnLvnGetdispinfoList(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    *pResult = 0;
    ...
    fxb::SizeFormat::Item *sItem = (fxb::SizeFormat::Item *)pDispInfo->item.lParam;
    if (sItem == XPR_NULL)
        return;
    ...
    if (XPR_TEST_BITS(sLvItem.mask, LVIF_TEXT))
    {
        ...
        case 1:
            {
                fxb::GetFormatedNumber(sItem->mSize, sLvItem.pszText, sLvItem.cchTextMax);
                break;
            }

        case 2:
            {
                const xpr_tchar_t *sText = fxb::SizeFormat::getDefUnitText(sItem->mSizeUnit);
                if (sText != XPR_NULL)
...
void SizeFormatDlg::OnLvnItemchangedList(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    *pResult = 0;
    ...
    {
        fxb::SizeFormat::Item *sItem = (fxb::SizeFormat::Item *)pNMLV->lParam;
        if (sItem != XPR_NULL)
        {
...
void SizeFormatDlg::OnLvnDeleteitemList(NMHDR* pNMHDR, LRESULT* pResult) 
{
    ...
    *pResult = 0;
    ...
    fxb::SizeFormat::Item *sItem = (fxb::SizeFormat::Item *)pNMListView->lParam;
    XPR_SAFE_DELETE(sItem);
}

xpr_sint_t SizeFormatDlg::addSizeFormat(fxb::SizeFormat::Item *aItem, xpr_sint_t aIndex)
{
    if (aItem == XPR_NULL)
        return -1;
...
void SizeFormatDlg::OnAdd(void)
{
    ...
    fxb::SizeFormat::Item *sItem = new fxb::SizeFormat::Item;
...
void SizeFormatDlg::OnModify(void)
{
    ...
    fxb::SizeFormat::Item *sItem = (fxb::SizeFormat::Item *)mListCtrl.GetItemData(sIndex);
    if (sItem == XPR_NULL)
        return;
