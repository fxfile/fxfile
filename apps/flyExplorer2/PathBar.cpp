...
void PathBar::setPath(LPITEMIDLIST aFullPidl)
{
    ...
    COM_FREE(mFullPidl);
    mFullPidl = fxb::CopyItemIDList(aFullPidl);

    if (XPR_IS_TRUE(mRealPathMode))
    {
        fxb::GetFolderPath(mFullPidl, mPath);
        if ((mPath[0] == '\0') || (mPath[0] == ':' && mPath[1] == ':'))
            fxb::GetName(mFullPidl, SHGDN_INFOLDER, mPath);
    }
    else
    {
        fxb::GetDispFullPath(mFullPidl, mPath);
    }

    if (XPR_IS_TRUE(mIsIcon))
        mIcon = fxb::GetItemIcon(mFullPidl);
...
void PathBar::OnPaint(void)
{
    ...
    COLORREF sBackColor = fxb::CalculateColor(GetSysColor(COLOR_WINDOW), GetSysColor(COLOR_3DFACE), 150);
    ...
    if (mPathMouse[0] != XPR_STRING_LITERAL('\0'))
    {
        ...
        sBackColor = fxb::CalculateColor(GetSysColor(COLOR_WINDOW), GetSysColor(COLOR_3DFACE), 150);
        ...
        else
        {
            ...
            else
            {
                ...
                if (sCount >= 0)
                {
                    ...
                    sFullPidl = fxb::CreatePidl(sSize + sizeof(xpr_ushort_t));
                    memcpy(sFullPidl, mFullPidl, sSize);
                }
                else
                {
                    sFullPidl = fxb::CreatePidl(sizeof(xpr_ushort_t));
                    sFullPidl->mkid.cb = 0;
                }
            }
...
void PathBar::trackContextMenu(CPoint aPoint)
{
    ...
    if (XPR_IS_TRUE(mRealPathMode))
        ...
        sFullPidl = fxb::GetDispFullPidl(sPath);
