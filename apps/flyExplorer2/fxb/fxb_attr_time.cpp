...
void AttrTime::addPath(const std::tstring &aPath)
{
...
xpr_bool_t AttrTime::OnRcsvDirAttrTimeProc(const std::tstring &strDir, DWORD sAttributes, xpr_sint_t sDepth)
{
    if (IsStop() == XPR_TRUE)
        return XPR_TRUE;

    std::tstring sPath = strDir + XPR_STRING_LITERAL('\\');
...
xpr_bool_t AttrTime::OnAttrTimeProc(const std::tstring &aPath, DWORD aAttributes)
{
    ...
    {
        fxb::SetFileTime(aPath.c_str(), mCreatedFileTime, mAccessFileTime, mModifiedFileTime);
    }
