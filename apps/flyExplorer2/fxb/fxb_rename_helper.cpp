...
void RenameHelper::setItem(LPSHELLFOLDER aShellFolder, LPITEMIDLIST aPidl, LPITEMIDLIST aFullPidl, xpr_ulong_t aShellAttributes)
{
    mShellFolder     = aShellFolder;
    mPidl            = aPidl;
    mFullPidl        = CopyItemIDList(aFullPidl);
...
RenameHelper::Result RenameHelper::rename(HWND aHwnd, const xpr_tchar_t *aNewName)
{
    ...
    if (XPR_IS_TRUE(sFileSystem))
    {
        ...
        else if (XPR_TEST_BITS(sFileAttributes, FILE_ATTRIBUTE_SYSTEM) || XPR_TEST_BITS(sFileAttributes, FILE_ATTRIBUTE_READONLY))
        {
            ...
            std::tstring sOldFileName = sOldPath.substr(sOldPath.rfind(XPR_STRING_LITERAL('\\'))+1);
            std::tstring sNewFileName = sNewPath.substr(sNewPath.rfind(XPR_STRING_LITERAL('\\'))+1);
        ...
        {
            LPITEMIDLIST sFullPidl = SHGetPidlFromPath(sNewPath.c_str());
            if (XPR_IS_NOT_NULL(sFullPidl))
            {
                LPSHELLFOLDER sShellFolder = XPR_NULL;
                LPITEMIDLIST sPidl = XPR_NULL;
                SH_BindToParent(sFullPidl, IID_IShellFolder, (LPVOID *)&sShellFolder, (LPCITEMIDLIST *)&sPidl);

                mNewPidl = CopyItemIDList(sPidl);
    ...
    SH_GetRealIDL(mShellFolder, mNewPidl, &mNewFullPidl);
