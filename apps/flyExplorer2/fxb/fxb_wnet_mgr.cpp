...
xpr_bool_t WnetMgr::isConnectedServer(const std::tstring &aNetPath)
{
    ...
    std::tstring sServer = aNetPath;
    xpr_size_t nFind = sServer.find(XPR_STRING_LITERAL('\\'), 2);
    if (nFind != std::tstring::npos)
        sServer.erase(nFind);
...
void WnetMgr::getNetFullPath(std::tstring &aFullPath)
{
    aFullPath.clear();

    std::tstring sPath(XPR_STRING_LITERAL("\\\\127.0.0.1"));
    LPITEMIDLIST sFullPidl = SHGetPidlFromPath(sPath.c_str());
