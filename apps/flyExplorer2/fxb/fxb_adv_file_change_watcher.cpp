...
#include "stdafx.h"
#include "fxb_adv_file_change_watcher.h"
...
public:
    ...
    AdvWatchId registerWatch(const std::tstring &aRootPath, AdvWatchItem *aNewAdvWatchItem)
    {
        ...
        if (XPR_IS_NULL(mDirectory))
        {
            ...
            std::tstring sDrivePath(aRootPath);
...
xpr_bool_t AdvFileChangeWatcher::create(void)
{
    ...
    mMonitorThread.SetThreadHandler(dynamic_cast<fxb::ThreadHandler *>(this));
    mNotifyThread.SetThreadHandler(dynamic_cast<fxb::ThreadHandler *>(this));
...
xpr_bool_t AdvFileChangeWatcher::getRootPath(const std::tstring &aPath, std::tstring &aRootPath)
{
    aRootPath.clear();
...
xpr_bool_t AdvFileChangeWatcher::OnThreadPreEntry(fxb::Thread &theThread)
{
    return XPR_TRUE;
}

unsigned AdvFileChangeWatcher::OnThreadEntryProc(fxb::Thread &theThread)
{
