...
{
    baseCSizingControlBar::OnWindowPosChanged(lpwndpos);
}
...
void CSizingControlBar::OnNcLButtonUp(UINT nFlags, CPoint point)
{
    baseCSizingControlBar::OnNcLButtonUp(nFlags, point);
}
...
void CSizingControlBar::NcCalcClient(LPRECT pRc, UINT nDockBarID)
{
	...
	if (nDockBarID == AFX_IDW_DOCKBAR_LEFT)
		rc.DeflateRect(0, 5, 4, 0);
	else if (nDockBarID == AFX_IDW_DOCKBAR_RIGHT)
		rc.DeflateRect(3, 5, 0, 0);
	else if (nDockBarID == AFX_IDW_DOCKBAR_TOP)
		rc.DeflateRect(3, 2, 0, 3);
	else if (nDockBarID == AFX_IDW_DOCKBAR_BOTTOM)
		rc.DeflateRect(2, 5, 0, 0);
	else
		rc.DeflateRect(0, 0, 2, 2);
...
void CSizingControlBar::OnNcPaint()
{
    ...
    mdc.FillRect(rcDraw, CBrush::FromHandle(
        (HBRUSH)(uintptr_t)GetClassLongPtr(m_hWnd, GCL_HBRBACKGROUND)));
...
void CSizingControlBar::EnableTracking(BOOL bTracking)
{
	m_bEnableTracking = bTracking;
}
...
void CSizingControlBar::LoadState(LPCTSTR lpszProfileName)
{
    ...
    TCHAR szSection[256];
    wsprintf(szSection, _T("%s-%d"), lpszProfileName, GetDlgCtrlID());
...
void CSizingControlBar::SaveState(LPCTSTR lpszProfileName)
{
    ...
    // MainFrame's OnClose() or DestroyWindow(), not in OnDestroy()
    ASSERT_VALID(this);
    ASSERT(GetSafeHwnd());
...
static LPCTSTR _fxSection   = _T("BarState");
static LPCTSTR _fxHorzSize  = _T("HorzSize");
static LPCTSTR _fxVertSize  = _T("VertSize");
static LPCTSTR _fxFloatSize = _T("FloatSize");

void CSizingControlBar::LoadStateIni(fxb::IniFile *pIniFile, LPCTSTR lpcszEntry/* = NULL*/)
{
	if (!pIniFile)
		return;

	if (!lpcszEntry)
		lpcszEntry = _fxSection;

	m_szHorz  = pIniFile->getValueZ(lpcszEntry, _fxHorzSize,  m_szHorz);
	m_szVert  = pIniFile->getValueZ(lpcszEntry, _fxVertSize,  m_szVert);
	m_szFloat = pIniFile->getValueZ(lpcszEntry, _fxFloatSize, m_szFloat);

    m_szHorz.cx  = max(m_szMinHorz.cx,  m_szHorz.cx);
    m_szHorz.cy  = max(m_szMinHorz.cy,  m_szHorz.cy);
    m_szVert.cx  = max(m_szMinVert.cx,  m_szVert.cx);
    m_szVert.cy  = max(m_szMinVert.cy,  m_szVert.cy);
    m_szFloat.cx = max(m_szMinFloat.cx, m_szFloat.cx);
    m_szFloat.cy = max(m_szMinFloat.cy, m_szFloat.cy);
}

void CSizingControlBar::SaveStateIni(fxb::IniFile *pIniFile, LPCTSTR lpcszEntry/* = NULL*/)
{
	if (!pIniFile)
		return;
    ...
    pIniFile->setValueZ(lpcszEntry, _fxHorzSize,  m_szHorz);
    pIniFile->setValueZ(lpcszEntry, _fxVertSize,  m_szVert);
    pIniFile->setValueZ(lpcszEntry, _fxFloatSize, m_szFloat);
}

void CSizingControlBar::LoadStateIni(LPCTSTR lpszProfileName)
{
    ASSERT_VALID(this);
    ASSERT(GetSafeHwnd()); // must be called after Create()

	TCHAR szPath[XPR_MAX_PATH + 1] = {0};
	fxb::GetModuleDir(szPath, XPR_MAX_PATH);
...
void CSizingControlBar::SaveStateIni(LPCTSTR lpszProfileName)
{
    // place your SaveState or GlobalSaveState call in
    // MainFrame's OnClose() or DestroyWindow(), not in OnDestroy()
    ASSERT_VALID(this);
	...
	_stprintf(szPath+_tcslen(szPath), _T("\\%s"), lpszProfileName);
	fxb::CreateDirectoryLevel(szPath);

	_stprintf(szPath+_tcslen(szPath), _T("\\Bar-%d.ini"), GetDlgCtrlID());
...
void CSizingControlBar::GlobalLoadStateIni(CFrameWnd* pFrame, LPCTSTR lpszProfileName)
{
    ...
    while (pos != NULL)
    {
        ...
        ASSERT(pBar != NULL);
        if (pBar->IsKindOf(RUNTIME_CLASS(CSizingControlBar)))
            pBar->LoadStateIni(lpszProfileName);
    }
}

void CSizingControlBar::GlobalSaveStateIni(CFrameWnd* pFrame, LPCTSTR lpszProfileName)
{
    POSITION pos = pFrame->m_listControlBars.GetHeadPosition();
    while (pos != NULL)
    {
        ...
        ASSERT(pBar != NULL);
        if (pBar->IsKindOf(RUNTIME_CLASS(CSizingControlBar)))
            pBar->SaveStateIni(lpszProfileName);
    }
}
...
// Bar Sync Horz Size
//----------------------------------------------------------------------
void CSizingControlBar::SetSyncHorz(CRuntimeClass *pBarClass, int cy)
{
    CSCBArray arrSCBars;
    GetRowSizingBars(arrSCBars);

	if (arrSCBars.GetSize() < 2)
		return;

	if (arrSCBars[0] == this &&
		arrSCBars[0]->IsKindOf(pBarClass) &&
		arrSCBars[1]->IsKindOf(pBarClass))
	{
		int cy2 = m_szVert.cy - cy;
		arrSCBars[0]->m_szVert.cy -= cy2;
		arrSCBars[1]->m_szVert.cy += cy2;
		
		m_pDockSite->DelayRecalcLayout();
	}
}

void CSizingControlBar::SetSyncHorz()
{
}

void CSizingControlBar::SetFloatingSize(CSize sz)
{
	SetFloatingSize(sz.cx, sz.cy);
}

void CSizingControlBar::SetFloatingSize(int cx, int cy)
{
	m_szFloat.cx = cx;
	m_szFloat.cy = cy;
