...
#include "fxb/fxb_multi_rename.h"
...
xpr_bool_t BatchRenameDlg::OnInitDialog(void) 
{
    ...
    addTabDialog(new RenTabDlg0, theApp.loadString(XPR_STRING_LITERAL("popup.batch_rename.tab.format")));
    addTabDialog(new RenTabDlg1, theApp.loadString(XPR_STRING_LITERAL("popup.batch_rename.tab.replace")));
    addTabDialog(new RenTabDlg2, theApp.loadString(XPR_STRING_LITERAL("popup.batch_rename.tab.insert")));
    addTabDialog(new RenTabDlg3, theApp.loadString(XPR_STRING_LITERAL("popup.batch_rename.tab.delete")));
    addTabDialog(new RenTabDlg4, theApp.loadString(XPR_STRING_LITERAL("popup.batch_rename.tab.case")));
    ...
    if (sResultApply        == XPR_TRUE) sFlags |= fxb::BatchRename::FlagsResultRename;
    if (sBatchFormatArchive == XPR_TRUE) sFlags |= fxb::BatchRename::FlagsBatchFormatArchive;
...
void BatchRenameDlg::addTabDialog(RenTabDlg *aDlg, const xpr_tchar_t *aTitle)
{
...
RenTabDlg *BatchRenameDlg::getTabDialog(xpr_sint_t aIndex)
{
...
void BatchRenameDlg::setBatchFormatText(const std::tstring &aBatchFormat, xpr_sint_t aSel)
{
...
void BatchRenameDlg::addPath(fxb::BatRenItem *aBatRenItem)
{
...
void BatchRenameDlg::OnLvnGetdispinfoList(NMHDR *pNMHDR, LRESULT *pResult)
{
    ...
    *pResult = 0;
    ...
    fxb::BatRenItem *sBatRenItem = (fxb::BatRenItem *)pDispInfo->item.lParam;
    if (sBatRenItem == XPR_NULL)
        return;
...
void BatchRenameDlg::OnSize(xpr_uint_t nType, xpr_sint_t cx, xpr_sint_t cy)
{
    ...
    {
        if (((RenTabDlg0 *)mTabDeque[0])->mTipDlg && ((RenTabDlg0 *)mTabDeque[0])->mTipDlg->m_hWnd)
        {
            CRect sRect, sClientRect;
            GetWindowRect(&sRect);
            ((RenTabDlg0 *)mTabDeque[0])->mTipDlg->GetWindowRect(&sClientRect);

            ((RenTabDlg0 *)mTabDeque[0])->mTipDlg->MoveWindow(sRect.right, sRect.top, sClientRect.Width(), sClientRect.Height());
        }
    }
...
void BatchRenameDlg::OnWindowPosChanged(WINDOWPOS FAR* lpwp) 
{
    ...
    if ((mTabDeque.empty() == false) && ((RenTabDlg0 *)mTabDeque[0])->mTipDlg != XPR_NULL && (lpwp->flags & SWP_NOMOVE) != SWP_NOMOVE)
    {
        CRect sRect, sClientRect;
        GetWindowRect(&sRect);
        ((RenTabDlg0 *)mTabDeque[0])->mTipDlg->GetWindowRect(&sClientRect);
...
void BatchRenameDlg::OnOK(void) 
{
    ...
    {
        fxb::MultiRename::Status uStatus = mBatchRename->getStatus();
        if (uStatus == fxb::MultiRename::StatusPreparing  ||
            uStatus == fxb::MultiRename::StatusValidating ||
            uStatus == fxb::MultiRename::StatusRenaming)
        {
            mBatchRename->stop();
            return;
        }
...
LRESULT BatchRenameDlg::OnFinalize(WPARAM wParam, LPARAM lParam)
{
    ...
    fxb::MultiRename::Status sStatus = mBatchRename->getStatus(&sPreparedCount, &sValidatedCount, &sRenamedCount);

    switch (sStatus)
    {
    case fxb::MultiRename::StatusRenameCompleted:
        {
    ...
    case fxb::MultiRename::StatusStopped:
        {
            mListCtrl.Invalidate(XPR_FALSE);
            mListCtrl.UpdateWindow();
    ...
    case fxb::MultiRename::StatusInvalid:
        {
            ...
            case fxb::MultiRename::ResultExcessPathLength:
            case fxb::MultiRename::ResultEmptiedName:
            case fxb::MultiRename::ResultEqualedName:
                {
                    mListCtrl.Invalidate(XPR_FALSE);
                    mListCtrl.UpdateWindow();
...
void BatchRenameDlg::OnTimer(UINT_PTR nIDEvent)
{
    if (nIDEvent == TM_ID_STATUS)
    {
        ...
        switch (sStatus)
        {
        case fxb::MultiRename::StatusPreparing:
            {
        ...
        case fxb::MultiRename::StatusValidating:
            {
        ...
        case fxb::MultiRename::StatusRenaming:
            {
...
void BatchRenameDlg::editItem(xpr_sint_t aIndex)
{
    ...
    fxb::BatRenItem *sBatRenItem = mBatchRename->getItem(aIndex);
    if (sBatRenItem == XPR_NULL)
        return;

    xpr_bool_t sNoChangeExt = mBatchRename->isFlag(fxb::BatchRename::FlagsNoChangeExt);
    ...
    {
        const xpr_tchar_t *sFindedExt = (const xpr_tchar_t *)fxb::GetFileExt(sName);
...
{
    xpr_bool_t sBatchFormatArchive = mBatchRename->isFlag(fxb::BatchRename::FlagsBatchFormatArchive);

    xpr_uint_t sFlags = mBatchRename->getFlags();
    if (sBatchFormatArchive == XPR_TRUE) sFlags &= ~fxb::BatchRename::FlagsBatchFormatArchive;
    else                                 sFlags |=  fxb::BatchRename::FlagsBatchFormatArchive;
...
void BatchRenameDlg::OnNoChangeExt(void) 
{
    ...
    xpr_uint_t sFlags = mBatchRename->getFlags();
    if (sNoChangeExt == XPR_TRUE) sFlags &= ~fxb::BatchRename::FlagsNoChangeExt;
    else                          sFlags |=  fxb::BatchRename::FlagsNoChangeExt;
...
{
    xpr_bool_t sResultRename = mBatchRename->isFlag(fxb::BatchRename::FlagsResultRename);

    xpr_uint_t sFlags = mBatchRename->getFlags();
    if (sResultRename == XPR_TRUE) sFlags &= ~fxb::BatchRename::FlagsResultRename;
    else                           sFlags |=  fxb::BatchRename::FlagsResultRename;
...
{
    pCmdUI->Enable(!mRenaming);
    pCmdUI->SetCheck(mBatchRename->isFlag(fxb::BatchRename::FlagsBatchFormatArchive));
}

void BatchRenameDlg::OnUpdateNoChangeExt(CCmdUI *pCmdUI)
{
    pCmdUI->Enable(!mRenaming);
    pCmdUI->SetCheck(mBatchRename->isFlag(fxb::BatchRename::FlagsNoChangeExt));
}

void BatchRenameDlg::OnUpdateResult(CCmdUI *pCmdUI)
{
    pCmdUI->Enable(!mRenaming);
    pCmdUI->SetCheck(mBatchRename->isFlag(fxb::BatchRename::FlagsResultRename));
}
...
void BatchRenameDlg::moveItem(xpr_sint_t aIndex, xpr_sint_t aNewIndex)
{
    ...
    fxb::BatRenItem *sBatRenItem1 = (fxb::BatRenItem *)mListCtrl.GetItemData(aIndex);
    fxb::BatRenItem *sBatRenItem2 = (fxb::BatRenItem *)mListCtrl.GetItemData(aNewIndex);
...
{
    RenTabDlg0 *sDlg = (RenTabDlg0 *)getTabDialog(0);
    if (sDlg == XPR_NULL || sDlg->m_hWnd == XPR_NULL)
        return;
    ...
    xpr_bool_t sNoChangeExt        = mBatchRename->isFlag(fxb::BatchRename::FlagsNoChangeExt);
    xpr_bool_t sResultApply        = mBatchRename->isFlag(fxb::BatchRename::FlagsResultRename);
    xpr_bool_t sBatchFormatArchive = mBatchRename->isFlag(fxb::BatchRename::FlagsBatchFormatArchive);
...
{
    RenTabDlg1 *sDlg = (RenTabDlg1 *)getTabDialog(1);
    if (sDlg == XPR_NULL || sDlg->m_hWnd == XPR_NULL)
        return;
    ...
    if (fxb::VerifyFileName(sOld) == XPR_FALSE || _tcslen(sOld) == 0)
    {
        const xpr_tchar_t *sMsg = theApp.loadString(XPR_STRING_LITERAL("popup.batch_rename.msg.wrong_filename"));
        MessageBox(sMsg, XPR_NULL, MB_OK | MB_ICONSTOP);
    ...
    if (fxb::VerifyFileName(sNew) == XPR_FALSE)
    {
        const xpr_tchar_t *sMsg = theApp.loadString(XPR_STRING_LITERAL("popup.batch_rename.msg.wrong_filename"));
        MessageBox(sMsg, XPR_NULL, MB_OK | MB_ICONSTOP);
...
{
    RenTabDlg2 *sDlg = (RenTabDlg2 *)getTabDialog(2);
    if (sDlg == XPR_NULL || sDlg->m_hWnd == XPR_NULL)
        return;
    ...
    if (fxb::VerifyFileName(sInsert) == XPR_FALSE || _tcslen(sInsert) == 0)
    {
        const xpr_tchar_t *sMsg = theApp.loadString(XPR_STRING_LITERAL("popup.batch_rename.msg.wrong_filename"));
        MessageBox(sMsg, XPR_NULL, MB_OK | MB_ICONSTOP);
...
{
    RenTabDlg3 *sDlg = (RenTabDlg3 *)getTabDialog(3);
    if (sDlg == XPR_NULL || sDlg->m_hWnd == XPR_NULL)
        return;
...
{
    RenTabDlg4 *sDlg = (RenTabDlg4 *)getTabDialog(4);
    if (sDlg == XPR_NULL || sDlg->m_hWnd == XPR_NULL)
        return;
