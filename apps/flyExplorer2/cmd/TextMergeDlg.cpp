...
#include "stdafx.h"
#include "TextMergeDlg.h"

#include "fxb/fxb_text_merge.h"
...
xpr_bool_t TextMergeDlg::OnInitDialog(void) 
{
    ...
    if (fxb::UserEnv::instance().mPlatformNT == XPR_FALSE)
...
void TextMergeDlg::OnOK(void) 
{
    if (mTextMerge != XPR_NULL)
    {
        if (mTextMerge->getStatus() == fxb::TextMerge::StatusMerging)
        {
            mTextMerge->Stop();
            return;
        }
...
LRESULT TextMergeDlg::OnFinalize(WPARAM wParam, LPARAM lParam)
{
    ...
    xpr_size_t sSucceeded = 0;
    fxb::TextMerge::Status sStatus = mTextMerge->getStatus(&sSucceeded);
    ...
    switch (sStatus)
    {
    case fxb::TextMerge::StatusFailed:
        {
    ...
    case fxb::TextMerge::StatusMergeCompleted:
        {
    ...
    case fxb::TextMerge::StatusStopped:
        {
