...
LRESULT BookmarkToolBar::OnBookmarkAsyncIcon(WPARAM wParam, LPARAM lParam)
{
    ...
    if (XPR_IS_RANGE(0, sIndex, fxb::BookmarkMgr::instance().getCount()-1))
    {
...
void BookmarkToolBar::createBookmarkBar(void)
{
    ...
    fxb::BookmarkMgr::instance().regAsyncIcon(*this, WM_BOOKMARK_ASYNC_ICON);
...
void BookmarkToolBar::addBookmarkButton(xpr_sint_t aIndex, fxb::BookmarkItem *aBookmarkItem)
{
    CToolBarCtrl &sToolBarCtrl = GetToolBarCtrl();
    ...
    fxb::BookmarkMgr::Result sResult = fxb::BookmarkMgr::instance().getIcon(aIndex, hIcon);
    if (hIcon)
...
xpr_bool_t BookmarkToolBar::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
    ...
    if (pnmhdr->code == TTN_GETDISPINFO)
    {
        if (gOpt->mConfig.mBookmarkTooltip == XPR_TRUE)
        {
            ...
            fxb::BookmarkItem *sBookmarkItem = fxb::BookmarkMgr::instance().getBookmark(sIndex);
            if (sBookmarkItem == XPR_NULL)
                return XPR_FALSE;
...
void BookmarkToolBar::setText(xpr_bool_t aText)
{
    if (mInit == XPR_TRUE)
    {
        ...
        else
        {
            ...
            for (i = 0; i < sCount; ++i)
            {
                sBookmarkItem = fxb::BookmarkMgr::instance().getBookmark(i);
...
DROPEFFECT BookmarkToolBar::OnDragOver(COleDataObject* aOleDataObject, DWORD aKeyState, CPoint aPoint)
{
    ...
    if (sBookmark != -1)
    {
        ...
        fxb::BookmarkItem *sBookmarkItem = sBookmarkMgr.getBookmark(sBookmark);
        if (sBookmarkItem != XPR_NULL)
        {
            xpr_bool_t sGetPidl = XPR_TRUE;
            LPITEMIDLIST sFullPidl = XPR_NULL;

            if (fxb::IsNetItem(sBookmarkItem->mPath.c_str()))
                sGetPidl = fxb::WnetMgr::instance().isConnectedServer(sBookmarkItem->mPath);

            if (sGetPidl == XPR_TRUE)
                sFullPidl = fxb::Path2Pidl(sBookmarkItem->mPath);

            if (sFullPidl != XPR_NULL && fxb::IsFileSystemFolder(sFullPidl))
            {
            ...
            else
            {
                ...
                fxb::SH_BindToParent(sFullPidl, IID_IShellFolder, (LPVOID *)&sShellFolder, (LPCITEMIDLIST *)&sPidl);
...
void BookmarkToolBar::OnDrop(COleDataObject *aOleDataObject, DROPEFFECT aDropEffect, CPoint aPoint)
{
    ...
    else
    {
        ...
        fxb::BookmarkItem *sBookmarkItem = fxb::BookmarkMgr::instance().getBookmark(mOldBookmark);
        if (mOldBookmark >= 0 && sBookmarkItem != XPR_NULL)
        {
            LPITEMIDLIST sFullPidl = fxb::Path2Pidl(sBookmarkItem->mPath);
            sFileOperation = (sFullPidl != XPR_NULL && fxb::IsFileSystemFolder(sFullPidl));
            COM_FREE(sFullPidl);
        }
...
void BookmarkToolBar::OnDropFileOperation(STGMEDIUM *aStgMedium, DROPEFFECT aDropEffect, xpr_sint_t aBookmark)
{
    ...
    fxb::BookmarkItem *sBookmarkItem = fxb::BookmarkMgr::instance().getBookmark(aBookmark);
    if (sBookmarkItem == XPR_NULL)
        return;
    ...
    LPITEMIDLIST sFullPidl = fxb::Path2Pidl(sBookmarkItem->mPath);
    if (sFullPidl != XPR_NULL && fxb::IsFileSystemFolder(sFullPidl) == XPR_TRUE)
    {
...
void BookmarkToolBar::OnDropShortcut(STGMEDIUM *aStgMedium, xpr_sint_t aBookmark)
{
    ...
    if (sFullPidl != XPR_NULL && fxb::IsFileSystemFolder(sFullPidl) == XPR_TRUE)
    {
        ...
        for (i = 0; i < sCount; ++i)
        {
            ...
            if (fxb::SetNewPath(sBookmarkPath, sDir, sName, XPR_STRING_LITERAL(".lnk")))
            {
                fxb::CreateShortcut(sBookmarkPath.c_str(), sFullPidl2);
            }
...
void BookmarkToolBar::OnDropExecFile(STGMEDIUM *aStgMedium, xpr_sint_t aBookmark)
{
    ...
    LPITEMIDLIST sFullPidl = fxb::Path2Pidl(sBookmarkItem->mPath);
    if (sFullPidl != XPR_NULL && fxb::IsFileSystem(sFullPidl) == XPR_TRUE && fxb::IsFileSystemFolder(sFullPidl) == XPR_FALSE)
    {
        xpr_sint_t sCount = sIda->cidl;
        LPITEMIDLIST sFullPidl2 = XPR_NULL; // fully-qualified PIDL
        LPITEMIDLIST sPidl1 = XPR_NULL; // folder PIDL
        ...
        for (i = 0; i < sCount; ++i)
        {
            ...
            {
                sPath[0] = '\"';
                fxb::GetName(sFullPidl2, SHGDN_FORPARSING, sPath+1);
                ...
                fxb::ExecFile(sBookmarkItem->mPath.c_str(), sStartup, sPath);
            }
...
void BookmarkToolBar::updateBookmarkButton(xpr_sint_t aIndex)
{
    ...
    sCount = fxb::BookmarkMgr::instance().getCount();
    if (aIndex == -1)
        aIndex = sCount - 1;

    fxb::BookmarkItem *sBookmarkItem = fxb::BookmarkMgr::instance().getBookmark(aIndex);
