...
public:
    fxb::FileScrap::Item *mFileScrapItem;
...
void FileScrapCtrl::initList(fxb::FileScrap::Group *aGroup, xpr_bool_t aRedraw)
{
    ...
    fxb::FileScrap::Item *sFileScrapItem;
    ...
    for (i = 0; sIterator != aGroup->mItemDeque.end(); ++sIterator, ++i)
    {
        ...
        if (fxb::IsFileSystemFolder(sFileScrapItem->mPath) == XPR_TRUE)
        {
            ++mDirCount;
        }
        else
...
void FileScrapCtrl::OnGetdispinfo(NMHDR *aNmHdr, LRESULT *aResult) 
{
    ...
    *aResult = 0;
        ...
        case 2:
            if (XPR_TEST_NONE_BITS(sItemData->mFileAttributes, FILE_ATTRIBUTE_DIRECTORY))
            {
                if (XPR_IS_TRUE(sItemData->mFileExist))
                    fxb::GetFileSize(sItemData->mFileSize, sLvItem.pszText, sLvItem.cchTextMax);
            }
            break;

        case 3:
            if (XPR_IS_TRUE(sItemData->mFileExist))
                fxb::GetFileType(sItemData->mFileScrapItem->mPath.c_str(), sLvItem.pszText);
            break;

        case 4:
            if (XPR_IS_TRUE(sItemData->mFileExist))
                fxb::GetFileTime(sItemData->mModifiedFileTime, sLvItem.pszText);
            break;
        }
    }
    ...
    if (sLvItem.mask & LVIF_IMAGE)
    {
        ...
        {
            const xpr_tchar_t *sExt = fxb::GetFileExt(sItemData->mFileName);
            ...
            {
                sLvItem.iImage = fxb::GetItemIconIndex(sItemData->mFileScrapItem->mPath.c_str());
            }
...
xpr_bool_t FileScrapCtrl::getSelPidls(LPSHELLFOLDER *aShellFolder, LPITEMIDLIST *aPidls, xpr_sint_t &aCount)
{
...
xpr_bool_t FileScrapCtrl::invokeCommandSelf(fxb::ContextMenu *aContextMenu, xpr_uint_t aId)
{
...
void FileScrapCtrl::executeLink(LPSHELLFOLDER aShellFolder, LPITEMIDLIST aPidl)
{
    xpr_tchar_t sPath[XPR_MAX_PATH + 1] = {0};
...
void FileScrapCtrl::executeUrl(LPSHELLFOLDER aShellFolder, LPITEMIDLIST aPidl)
{
    HRESULT sHResult;
...
void FileScrapCtrl::executeLinkFolder(LPITEMIDLIST aFullPidl)
{
    if (XPR_IS_NOT_NULL(mObserver))
    {
...
void FileScrapCtrl::doExecuteError(LPSHELLFOLDER aShellFolder, LPITEMIDLIST aPidl)
{
    xpr_tchar_t sPath[XPR_MAX_PATH + 1];
...
{
    fxb::FileScrap::Group *sGroup = mGroup;
...
xpr_sint_t CALLBACK FileScrapCtrl::DefaultItemCompareProc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
    ...
    else if (sColumn == 3)
    {
        if ((sItemData1->mFileAttributes & FILE_ATTRIBUTE_DIRECTORY) == (sItemData2->mFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
        {
            ...
            {
                xpr_tchar_t *sExt1 = (xpr_tchar_t *)fxb::GetFileExt(sItemData1->mFileName);
                xpr_tchar_t *sExt2 = (xpr_tchar_t *)fxb::GetFileExt(sItemData2->mFileName);
...
void FileScrapCtrl::removeItems(const fxb::FileScrap::IdSet &aIdSet)
{
    xpr_sint_t i;
    xpr_sint_t sCount;
    ItemData *sItemData;
...
void FileScrapCtrl::getSelFileScrapItemIdSet(fxb::FileScrap::IdSet &sIdSet)
{
    xpr_sint_t sIndex;
    POSITION sPosition;
    ItemData *sItemData;
